...
class SamFotoAQController
{
    private SamFotoAQService $samfotoAQService;

    public function __construct(SamFotoAQService $samfotoAQService)
    {
        $this->samfotoAQService = $samfotoAQService;
    }
    ...
    //         $data = $this->mypicturesService->mypictures($fechad,$fechah,$location);
    ...
    public function createModel(Request $request, Response $response, string $tabla): Response
    {
        ...
        try {
            ...
            $model = $this->samfotoAQService->createModel($table);
        } catch (Throwable $ex) {
