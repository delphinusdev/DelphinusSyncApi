...
class SamFotoAQService
{
    private SqlSrvGenericContext $samfotoaqContext;
    private GenerateModeloService $modelo;

    public function __construct(SqlSrvGenericContext $samfotoaqContext, GenerateModeloService $modelo)
    {
        $this->samfotoaqContext = $samfotoaqContext;
        $this->modelo = $modelo;
         error_log("SamFotoAQService __construct - PDO Hash: " . spl_object_hash($this->samfotoaqContext->getPdo()));
    }
    public function createModel($table)
    {

        $query = $this->modelo->getTableColumns($table)->build();
        $tabla = $this->samfotoaqContext->select($query['sql'], $query['params']);
    ...
    public function clouds(string $fechad, string $fechah, array $location,string $tipo = 'clouds'): array
    {
        ...
        try {
            ...
            if ($tempTableName) {
               $this->samfotoaqContext->executeStatement(sprintf("DROP TABLE %s;", $tempTableName));
            }
        }
        return $preparedIds;
    }
    ...
    public function thumbs(string $fechad, string $fechah, array $location): array
    {
        ...
        return $this->samfotoaqContext->select($query['sql'], $query['params']);
    }
    ...
    private function fetchPedidoData(string $fechad, string $fechah, array $location,string $tipo): array
    {
        ...
        return $this->samfotoaqContext->select($pedidosQuery['sql'], $pedidosQuery['params']);
    }
    ...
    private function createAndPopulateTempTable(array $photoIds): string
    {
        ...
        $this->samfotoaqContext->executeStatement($createTempTableSql);
        ...
        foreach ($photoIdBatches as $batch) {
            ...
            $this->samfotoaqContext->executeStatement($insertSql, $batch);
        }
    ...
    private function fetchPhotosWithTempTable(string $tempTableName, array $groupIds, string $locationId, string $location_code,string $tipo): array
    {
        ...
        return $this->samfotoaqContext->select($finalQuery, $finalParams);
    }
