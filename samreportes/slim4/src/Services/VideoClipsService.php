    ...
    public function tmdVideos(string $fechad, string $fechah, array $location): array
{
    $query = pedidos::from('p')
        ...
        ->select([
            ...
            sprintf("ISNULL(%s,'NA')", grupos::Video(null, 'c')) . " AS uniqid", 
        ...
        // --- INICIO DE CAMBIOS ---
        // Se añaden los nuevos campos al GROUP BY para que la consulta sea válida
        ->groupBy(
            [
    ...
    public function clips($fechad, $fechah, $location): array
{
    $query = pedidos::from('p')
        ->newQuery()
        // --- INICIO DEL CAMBIO ---
        // Se añaden los campos faltantes para mantener una estructura consistente.
        ->select([
            pedidos::IDVENTA('IdVenta', 'p'),
            pedidos::IDPEDIDO('IdPedido', 'p'),
            "NULL AS ConfirmaId",
